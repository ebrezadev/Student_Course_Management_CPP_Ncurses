...
StateNotificationTheoreticalCourseAddFail::StateNotificationTheoreticalCourseAddFail()
{
...
void StateNotificationTheoreticalCourseAddFail::formatHandler()
{
...
void StateNotificationTheoreticalCourseAddFail::mainWindowHandler()
{
	next_state = State_list_t::State_Notification_Theoretical_Course_Add_Fail;
	...
	if (screen_update_flag)
	{
		if (window != nullptr)
		{
			...
			mvwprintw(window, NOTIFICATION_ROWS - 7, (NOTIFICATION_COLS - strlen("Please enter valid values.")) / 2, "Please enter valid values.");
			wattron(window, COLOR_PAIR(NOTIFICATION_FAIL_BUTTON_COLOR_ON));
			mvwprintw(window, NOTIFICATION_ROWS - 5, (NOTIFICATION_COLS - strlen("   BACK   ")) / 2, "   BACK   ");
			wattroff(window, COLOR_PAIR(NOTIFICATION_FAIL_BUTTON_COLOR_ON));
		}
	}
}

void StateNotificationTheoreticalCourseAddFail::inputHandler(int input)
{
	switch (input)
	{
	case (int)13: // ENTER
