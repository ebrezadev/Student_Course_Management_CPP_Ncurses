...
StateNotificationGradeEntryFail::StateNotificationGradeEntryFail()
{
...
void StateNotificationGradeEntryFail::formatHandler()
{
...
void StateNotificationGradeEntryFail::mainWindowHandler()
{
	next_state = State_list_t::State_Notification_Grade_Entry_Fail;
	...
	if (screen_update_flag)
	{
		if (window != nullptr)
		{
			...
			mvwprintw(window, NOTIFICATION_ROWS - 7, (NOTIFICATION_COLS - strlen("Invalid grade.")) / 2, "Invalid grade.");
...
void StateNotificationGradeEntryFail::inputHandler(int input)
{
	switch (input)
	{
	case KEY_UP:
