...
StateNotificationUnregisterSuccess::StateNotificationUnregisterSuccess()
{
...
void StateNotificationUnregisterSuccess::formatHandler()
{
	...
	{
		footerWindowRender((char *)"OKAY: UNREGISTER MENU", 1, max_columns, max_rows - 1, 0, NOTIFICATION_FOOTER_BAR_COLOR);
	}
...
void StateNotificationUnregisterSuccess::mainWindowHandler()
{
	next_state = State_list_t::State_Notification_Unregister_Success;
	...
	if (screen_update_flag)
	{
		if (window != nullptr)
		{
			...
			mvwprintw(window, NOTIFICATION_ROWS - 7, (NOTIFICATION_COLS - strlen("Unregistered successfuly.")) / 2, "Unregistered successfuly.");
...
void StateNotificationUnregisterSuccess::inputHandler(int input)
{
	switch (input)
	{
	case KEY_UP:
