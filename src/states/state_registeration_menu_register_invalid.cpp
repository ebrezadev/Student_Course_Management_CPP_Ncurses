...
StateNotificationRegisterFail::StateNotificationRegisterFail()
{
...
void StateNotificationRegisterFail::formatHandler()
{
...
void StateNotificationRegisterFail::mainWindowHandler()
{
	next_state = State_list_t::State_Notification_Register_Fail;
	...
	if (screen_update_flag)
	{
		if (window != nullptr)
		{
			...
			mvwprintw(window, NOTIFICATION_ROWS - 7, (NOTIFICATION_COLS - strlen("Registered already.")) / 2, "Registered already.");
...
void StateNotificationRegisterFail::inputHandler(int input)
{
	switch (input)
	{
	case KEY_UP:
