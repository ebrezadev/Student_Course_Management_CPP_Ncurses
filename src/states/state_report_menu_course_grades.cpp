...
StateCourseGrades::StateCourseGrades()
{
...
void StateCourseGrades::formatHandler()
{
	getmaxyx(stdscr, max_rows, max_columns);

	if (!title_window_is_rendered)
	{
		titleWindowRender((char *)"WORKSPACE: COURSE GRADES", TITLE_BAR_WIDTH, max_columns - MENU_COLS, 0, MENU_COLS, WORKSPACE_TITLE_BAR_COLOR_ON);
	}
...
void StateCourseGrades::mainWindowHandler()
{
	next_state = State_list_t::State_Course_Grades;
	...
	if (!done_calculations && window != nullptr)
	{
		...
		int student_list_cols = max_columns - 14;
	...
	if (screen_update_flag)
	{
		...
		else
		{
			...
			else if (done_calculations && course_selected && portal.courseRegisterListLength(course_current_index) != 0)
			{
				...
				mvwprintw(window, GRADES_REPORT_LIST_GUIDE_LINE_FROM_TOP, 3 + strlen("Index") + student_last_name_cols + student_first_name_cols + student_id_cols, "Grade");

				mvwaddch(window, GRADES_REPORT_LIST_GUIDE_LINE_FROM_TOP, 2 + strlen("Index") + student_last_name_cols + student_first_name_cols + student_id_cols + strlen("Grade "), ACS_VLINE);
				...
				for (size_t index = 0; index < page_size; index++)
				{
					...
					if (grade != GRADES_NOT_ENTERED)
					{
						mvwprintw(window, index + GRADES_REPORT_LIST_BEGIN_LINE_FROM_TOP, 3 + strlen("Index") + student_last_name_cols + student_first_name_cols + student_id_cols, "%.1f", grade);
					}
					else
					{
						mvwprintw(window, index + GRADES_REPORT_LIST_BEGIN_LINE_FROM_TOP, 3 + strlen("Index") + student_last_name_cols + student_first_name_cols + student_id_cols, "%s", "NULL");
					}
					mvwaddch(window, index + GRADES_REPORT_LIST_BEGIN_LINE_FROM_TOP, 2 + strlen("Index") + student_last_name_cols + student_first_name_cols + student_id_cols + strlen("Grade "), ACS_VLINE);
...
void StateCourseGrades::inputHandler(int input)
{
	switch (input)
	{
	case KEY_UP:
