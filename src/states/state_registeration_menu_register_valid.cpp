...
StateNotificationRegisterSuccess::StateNotificationRegisterSuccess()
{
...
void StateNotificationRegisterSuccess::formatHandler()
{
...
void StateNotificationRegisterSuccess::mainWindowHandler()
{
	next_state = State_list_t::State_Notification_Register_Success;
	...
	if (screen_update_flag)
	{
		if (window != nullptr)
		{
			...
			mvwprintw(window, NOTIFICATION_ROWS - 7, (NOTIFICATION_COLS - strlen("Registered successfuly.")) / 2, "Registered successfuly.");
...
void StateNotificationRegisterSuccess::inputHandler(int input)
{
	switch (input)
	{
	case KEY_UP:
